...
void setup_logging() {
    // Check if a logger for the classifier already exists to avoid re-creating it.
    if (spdlog::get("pto")) {
        return;
    }

    auto logger = spdlog::stdout_color_mt("pto");
    ...
    logger->info("C++ logging for PairTree Orchard initialized.");
}
...
PYBIND11_MODULE(PairtreeOrchard, m) {
    ...
    m.doc() = "pybind11 plugin for PairtreeOrchard";
